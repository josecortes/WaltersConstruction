require 'spec_helper'

# This spec was generated by rspec-rails when you ran the scaffold generator.
# It demonstrates how one might use RSpec to specify the controller code that
# was generated by Rails when you ran the scaffold generator.
#
# It assumes that the implementation code is generated by the rails scaffold
# generator.  If you are using any extension libraries to generate different
# controller code, this generated spec may or may not pass.
#
# It only uses APIs available in rails and/or rspec-rails.  There are a number
# of tools you can use to make these specs even more expressive, but we're
# sticking to rails and rspec-rails APIs to keep things simple and stable.
#
# Compared to earlier versions of this generator, there is very limited use of
# stubs and message expectations in this spec.  Stubs are only used when there
# is no simpler way to get a handle on the object needed for the example.
# Message expectations are only used when there is no simpler way to specify
# that an instance is receiving a specific message.

describe HoursController do

  # This should return the minimal set of attributes required to create a valid
  # Hour. As you add validations to Hour, be sure to
  # update the return value of this method accordingly.
  def valid_attributes
    {}
  end

  describe "GET index" do
    it "assigns all hours as @hours" do
      hour = Hour.create! valid_attributes
      get :index
      assigns(:hours).should eq([hour])
    end
  end

  describe "GET show" do
    it "assigns the requested hour as @hour" do
      hour = Hour.create! valid_attributes
      get :show, :id => hour.id
      assigns(:hour).should eq(hour)
    end
  end

  describe "GET new" do
    it "assigns a new hour as @hour" do
      get :new
      assigns(:hour).should be_a_new(Hour)
    end
  end

  describe "GET edit" do
    it "assigns the requested hour as @hour" do
      hour = Hour.create! valid_attributes
      get :edit, :id => hour.id
      assigns(:hour).should eq(hour)
    end
  end

  describe "POST create" do
    describe "with valid params" do
      it "creates a new Hour" do
        expect {
          post :create, :hour => valid_attributes
        }.to change(Hour, :count).by(1)
      end

      it "assigns a newly created hour as @hour" do
        post :create, :hour => valid_attributes
        assigns(:hour).should be_a(Hour)
        assigns(:hour).should be_persisted
      end

      it "redirects to the created hour" do
        post :create, :hour => valid_attributes
        response.should redirect_to(Hour.last)
      end
    end

    describe "with invalid params" do
      it "assigns a newly created but unsaved hour as @hour" do
        # Trigger the behavior that occurs when invalid params are submitted
        Hour.any_instance.stub(:save).and_return(false)
        post :create, :hour => {}
        assigns(:hour).should be_a_new(Hour)
      end

      it "re-renders the 'new' template" do
        # Trigger the behavior that occurs when invalid params are submitted
        Hour.any_instance.stub(:save).and_return(false)
        post :create, :hour => {}
        response.should render_template("new")
      end
    end
  end

  describe "PUT update" do
    describe "with valid params" do
      it "updates the requested hour" do
        hour = Hour.create! valid_attributes
        # Assuming there are no other hours in the database, this
        # specifies that the Hour created on the previous line
        # receives the :update_attributes message with whatever params are
        # submitted in the request.
        Hour.any_instance.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => hour.id, :hour => {'these' => 'params'}
      end

      it "assigns the requested hour as @hour" do
        hour = Hour.create! valid_attributes
        put :update, :id => hour.id, :hour => valid_attributes
        assigns(:hour).should eq(hour)
      end

      it "redirects to the hour" do
        hour = Hour.create! valid_attributes
        put :update, :id => hour.id, :hour => valid_attributes
        response.should redirect_to(hour)
      end
    end

    describe "with invalid params" do
      it "assigns the hour as @hour" do
        hour = Hour.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Hour.any_instance.stub(:save).and_return(false)
        put :update, :id => hour.id, :hour => {}
        assigns(:hour).should eq(hour)
      end

      it "re-renders the 'edit' template" do
        hour = Hour.create! valid_attributes
        # Trigger the behavior that occurs when invalid params are submitted
        Hour.any_instance.stub(:save).and_return(false)
        put :update, :id => hour.id, :hour => {}
        response.should render_template("edit")
      end
    end
  end

  describe "DELETE destroy" do
    it "destroys the requested hour" do
      hour = Hour.create! valid_attributes
      expect {
        delete :destroy, :id => hour.id
      }.to change(Hour, :count).by(-1)
    end

    it "redirects to the hours list" do
      hour = Hour.create! valid_attributes
      delete :destroy, :id => hour.id
      response.should redirect_to(hours_url)
    end
  end

end
